...
begin  // 对于简体中文，为系统卸载管理处写入中文信息
  if (IsNeedSetUninstName = true) and
     (MathType{#MyAppMajorVersion}Main = true) and
     (languageName = 'chinesesimp') and
     (RegValueExists(HKLM32, 'SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\DSMT{#MyAppMajorVersion}', 'DisplayName')) then
  begin
