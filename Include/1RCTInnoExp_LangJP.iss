...
japanese.RCTISEFeatureIsExp=実験機能
japanese.RCTISEFeatureIsDebug=デバッグ
...
japanese.RCTISEFileFormatSupport=%1 形式のサポート
japanese.RCTISELibrarySupport=%1 サポートライブラリ
...
japanese.RCTISERunRegApp=アプリケーション登録を行っています。しばらくお待ちください...
japanese.RCTISERunRegFonts=フォントの登録を行っています。しばらくお待ちください...
japanese.RCTISEEnablePremium=プレミアム機能を有効にする
...
japanese.RCTMsgDebugNotice=このバージョンのアプリケーションはテスト専用です。
...
japanese.RCTMsgOSNotMeetRequirement=コンピュータが実行しているオペレーティング システムが要件を満たしていません (%1)。
japanese.RCTMsgOSRequirement=このプログラムには %1 OS が必要です。
japanese.RCTMsgOSMinRequirement=このアプリは %1 以上の OS が必要です。
japanese.RCTMsgOSNotSupportNET48=現在お使いのコンピュータで実行されている Windows バージョンは、Microsoft .NET Framework 4.8 をサポートしていません。このため、アプリケーションの一部のコンポーネントで問題が発生する可能性があります。問題を解決するには、オペレーティングシステムを Windows 10 バージョン 1607 以降にアップグレードしてください。
japanese.RCTMsgAppMayNotFuncOnThisOS=アプリはこのオペレーティングシステムで動作する可能性がありますが、予期しない問題が発生しないことを保証することはできません。
japanese.RCTMsgAppTryContinueInst=現在のオペレーティングシステムのバージョンは公式にはサポートされていません。そのため、現在のオペレーティングシステムへのインストールサポートは実験的な機能です。japanese.RCTMsgMustUninstExistVer=インストールする前に、既存のバージョンのアプリをアンインストールしてください。
japanese.RCTMsgMustUninstExistVer=インストールする前に、アプリケーションの既存のバージョンをアンインストールしてください。
japanese.RCTMsgPatchAppNotExistNotice=コンピューター上の %1 アプリを検出できませんでした。
japanese.RCTMsgPatchAppNotExistChk=アプリが正しくインストールされていることを確認してから、セットアップを再度実行します。
japanese.RCTMsgWinStoreSvcChkFailed=Microsoft Store アプリの展開に使用される必要なサービスの 1 つ以上が無効になっているため、%1 のインストールに影響を与える可能性があります。 インストールを続行する前に、これらのサービスを有効にすることを強くお勧めします。
...
japanese.RCTMsgAppStillRunning=セットアップは、%1 がまだ実行中であることを検出しました。
...
japanese.RCTMsgNotSupportSilent=このアプリケーションはサイレントインストールに対応していません。セットアップを直接実行してください。
japanese.RCTMsgPleaseContactUs=何か問題がございましたら、お問い合わせください。よろしくお願いいたします。
...
//japanese.WDrvPreInstChkAskUser=デバイスマネージャーで上記のエラーのいずれかを見つけましたか？
//japanese.WDrvPreInstChkSameErrFound=同じエラー問題を見つけました。
...
japanese.WDrvErrorCannotContinue=エラー：%nデバイスの正常な動作に影響を与える重大な問題が発生しました。ガイドに従って解決し、その後セットアップを再度実行してください。
japanese.WDrvFMConfHasError=お使いのコンピューターの現在の UEFI BIOS 構成に問題があります。
japanese.WDrvGPUStateError=グラフィックドライバーが正常に機能していないことが検出されました。GPU に問題がないか確認してください。%n%nこのプロジェクトのリリースノートにトラブルシューティングのガイドが記載されている場合がありますので、参照してください。//japanese.WDrvFMBootModeHowTo=Windows RE、システムインストールメディア、MBR2GPT などのツールを使用して、システムが存在するハードドライブのパーティションテーブルを GPT/GUID 形式に変換してください。その後、システムブート修復を実行して、UEFI ブートモードへの変換を完了してください。
japanese.WDrvFMBootModeHowTo=Windows 回復環境／インストールメディア、MBR2GPT ユーティリティなどのツールを使用して、オペレーティングシステムがインストールされているハードディスクのパーティションテーブルを GPT／GUID 形式に変換してください。その後、必ずシステムブート修復を実行し、UEFI ブートモードへの変換を完了させてください。
...
japanese.WDrvFMBootModeHowToASUSExtra=特定の ASUS マザーボードモデル（H81、B85 など）では、UEFI BIOS 設定で「Primary Display」や「iGPU Multi-Monitor」などの設定を編集する必要もあります。
...
japanese.WDrvPolicyDenyXSignNotice=Windows ドライバーポリシーは強制モードで有効になっているため、セットアップを続行できません。オペレーティングシステムから Windows ドライバーポリシーを削除してください。%n%nこのプロジェクトのリリースノートで詳細情報を確認できます。
