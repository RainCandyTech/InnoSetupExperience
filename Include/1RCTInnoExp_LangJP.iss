...
;japanese.RCTMsgAppTryContinueInst=The current version of the operating system is not officially supported, so installation support on the current operating system is an experimental feature.
japanese.RCTMsgMustUninstExistVer=インストールする前に、既存のバージョンのアプリをアンインストールしてください。
...
//japanese.RCTMsgNotSupportSilent=This application does not support silent installation. Please run setup directly.
//japanese.RCTMsgPleaseContactUs=Please contact us for any problems. Thank you!
...
//japanese.WDrvFMBootModeHowTo=Windows RE、システムインストールメディア、MBR2GPT などのツールを使用して、システムが存在するハードドライブのパーティションテーブルを GPT/GUID 形式に変換してください。その後、システムブート修復を実行して、UEFI ブートモードへの変換を完了してください。
