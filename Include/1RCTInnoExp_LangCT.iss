...
chinesetrad.RCTISERunRegApp=正在註冊應用程式，請稍候...
chinesetrad.RCTISERunRegFonts=正在註冊字體，請稍候...
...
//chinesetrad.WDrvPreInstChkNowNotice=現在，您需要打開裝置管理員，檢查「顯示卡」分類中是否存在出現感嘆號圖示的裝置。%n如有，請按兩下圖示，查看其中是否存在「代碼 %1」的報錯信息。%n在正常情況下，此報錯問題不應該出現。
chinesetrad.WDrvErrorCannotContinue=錯誤：%n遇到了影響裝置正常運作的嚴重問題，請依照指南解決，然後重新執行安裝程式。
chinesetrad.WDrvFMConfHasError=計算機當前的 UEFI BIOS 配置可能存在問題。
chinesetrad.WDrvGPUStateError=偵測到顯示驅動程式目前沒有正常運作，請對顯示卡進行問題檢查。%n%n本專案的發行說明中可能提供了疑難排解指南，敬請參閱。
...
chinesetrad.WDrvFMBootModeDisableCSM=請進入 UEFI BIOS 設定程式，找到並關閉「相容性支援模組」（簡稱 CSM）選項。
chinesetrad.WDrvFMBootModeLegacyDisableWarning=※ 重要：如果沒有執行第一步就關閉該選項的話，將無法啟動作業系統。
chinesetrad.WDrvFMBootModeHowToASUSExtra=對於某些特定的 ASUS 主機板型號（H81、B85 等），您還需要在 UEFI BIOS 設定程式中對某些選項進行調整，比如「首選顯示卡」和「初始化 iGPU」選項。
...
chinesetrad.WDrvFMConfErrIGFXNotWorking=此問題將會導致整合式顯示卡和不支持顯示輸出的 GPU 裝置（如 NVIDIA P106-100 和 CMP 40HX）無法正常工作。
