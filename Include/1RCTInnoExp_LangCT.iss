...
//chinesetrad.WDrvPreInstChkSameErrFoundDesc=了解接下來應該怎麼做。
chinesetrad.WDrvPreInstChkSameErrNotFound=我沒有發現問題，或者報錯問題與前面提到的不一樣。
//chinesetrad.WDrvPreInstChkSameErrFoundDesc=安裝程序將繼續。其他的報錯問題在安裝驅動後也許就解決了。
chinesetrad.WDrvPreInstChkNowNotice=現在，您需要打開「裝置管理員」，檢查「顯示卡」分類中是否存在出現感嘆號圖標的設備。%n如有，請按兩下圖標，查看其中是否存在「代碼 %1」的報錯信息。
...
chinesetrad.WDrvFMBootModeDisableCSM=請轉到 UEFI BIOS 設置界面，找到並關閉「兼容性支持模塊」（簡稱 CSM）選項。
